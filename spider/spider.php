...
if (!file_exists(__DIR__ . '/../content/ytwizard/vendor/autoload.php')) {
  throw new \Exception('please run "composer require google/apiclient:~2.0" in "' . __DIR__ .'"');
}
require __DIR__ . '/../dao/youtubeDAO.php';
require_once __DIR__ . '/../content/ytwizard/vendor/autoload.php';
...
for($i=0; $i<sizeof($data);$i++){
	...
	$vsize = array();
$info2 = getUdemyInfo($data[$i]["linkAffiliate"]);
 $link= $info2['video'];
 $shortenLink = $data[$i]["shortenLinkAffiliate"];
 $title= $info2["title"];
  /* if (strpos($link, 'mp4') == false) {	 
	updateYtUpload("echec", $title);  
    continue;
}*/
 $newLine = "
";	
$desc = $shortenLink.$newLine.$shortenLink.$newLine.$newLine.$info2['desc'];
 $size=$info2['size'];
  if (strpos($link, 'mp4') == false  || empty($size)==true ||  $size<10000 ) {
	updateYtUpload("echec", $shortenLink);
    continue;
}
 $thumb=$data[$i]["thumb"];
 ...
 $tags= array("udemy", "udemy course", "online tutorial", "online learning", "udemy review", "udemy coupon", "udemy voucher", "udemy discount");
  ...
  try{
   ...
   //$media->setFileSize($s); 
	...
	echo $title." ------ ".$status['snippet']['title'];
     if(trim($title)==trim($status['snippet']['title'])){
		updateYtUpload("uploaded", $shortenLink); 
	  }else{
		updateYtUpload("echec", $shortenLink);
		if(empty($status['id'])) continue;
	  }
	$videoId = $status['id'];
...
//die(var_dump($status));
    ...
    $snippet = new Google_Service_YouTube_VideoSnippet();
    $snippet->setTitle($title);
    $snippet->setDescription("Get the course here".$newLine.$shortenLink.$newLine.$shortenLink);
    $snippet->setTags($tags);
	//array("tag1", "tag2")
    // Numeric video category. See
    // https://developers.google.com/youtube/v3/docs/videoCategories/list
